...
and \mintinline{julia}/damping/, which the the damping factor-- the assumed probability that a graph traveler will stop traveling on any given move.

\subsubsection{PageRank Matrix Generation}

The function \mintinline{julia}/pagerank_matrix/ generates a Markov Matrix that will be used in the PageRank algorithm.
...
Designates the calculation method we will use-- either \mintinline{julia}/"iter"/ for the iterative method or \mintinline{julia}/"epsi"/ for the epsilon method.
...
For PageRank, the function \mintinline{julia}/print_pagerank/ prints the Vertices with the top PageRank scores to standard output.
